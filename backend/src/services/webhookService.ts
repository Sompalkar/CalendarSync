...
import { getFullWebhookUrl } from "../config/webhook";
...
export const setupWebhook = async (userId: string): Promise<void> => {
  try {
    ...
    const webhookUrl = await getFullWebhookUrl();
    ...
    user.webhookResourceId = response.data.resourceId || undefined;
    user.webhookExpiration = new Date(
      Number.parseInt(response.data.expiration || "0")
    );
    await user.save();
